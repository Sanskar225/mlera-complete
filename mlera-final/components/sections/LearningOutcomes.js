            ...
            <AnimatedSection key={o.title} delay={80 + (i % 3) * 80}>
              <div className="group h-full bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 card-hover">
                <div className="flex items-center justify-between mb-5">
                  <span
                    className="text-[10px] font-syne font-bold uppercase tracking-widest px-2.5 py-1 rounded-full border"
                    style={{ color: o.color, borderColor: `${o.color}35`, background: `${o.color}10` }}
                  >
                    {o.tag}
                  </span>
                  <span className="text-2xl">{o.icon}</span>
                </div>
                <h3 className="font-syne font-bold text-[0.95rem] leading-snug mb-3 group-hover:text-white transition-colors">
                  {o.title}
                </h3>
        ...
        <AnimatedSection delay={200}>
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-3xl p-8 md:p-10">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 mb-8">
              ...
              <span className="bg-violet/10 border border-violet/25 text-violet text-xs font-syne font-bold px-3 py-1 rounded-full tracking-wider uppercase">
                Structured Path
              </span>
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {MILESTONES.map((m, i) => (
                  <div key={m.week} className="flex flex-col items-center text-center relative">
                    <div
                      className="w-10 h-10 rounded-full border-2 flex items-center justify-center text-xs font-syne font-bold mb-3 z-10 bg-[var(--surface)]"
                      style={{ borderColor: m.color, color: m.color, boxShadow: `0 0 16px ${m.color}30` }}
                    >
                      {i + 1}
                    </div>
            ...
            <div className="mt-8 pt-6 border-t border-[var(--border)] flex flex-col sm:flex-row items-start sm:items-center gap-4 flex-wrap">
              {[
                ...
                <div key={text} className={`flex items-center gap-2 ${color} text-sm font-dm`}>
